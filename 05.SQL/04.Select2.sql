...
select custid, count(*), avg(saleprice) from orders 
        ...
        group by custid
        having count(*) >= 2;   -- 순서 유의 where 다음에 group by
-- 누가 책 매출액이 많은가?
select custid, sum(saleprice) from orders 
        group by custid 
        order by sum(saleprice) desc; -- group by 다음에 order by
        
/*
 * 1.5 두 개 이상의 테이블
 */
-- custid와 custid_1이 일치하는 field가 의미가 있음. 
...
select c.custid, c.name, c.address, c.phone, o.bookid, o.saleprice, o.orderdate
        from customer c
        join orders o     -- inner 생략 가능
        on c.custid = o.custid
        order by c.custid;
-- 고객의 이름과 고객이 주문한 도서의 판매가격
select c.name, o.saleprice from customer c  -- c.name, o.saleprice 테이블명 안써도 됨
        join orders o
        on c.custid = o.custid;
...
select c.custid, c.name, o.saleprice
        from customer c
        left join orders o        -- outer 생략 가능
        on c.custid = o.custid;
...
select bookname, price from book
        where price = (select max(price) from book);    -- oracle에서는 많이 사용
...
select name from customer 
        where custid like (select custid from orders where bookid in (select bookid from book where publisher like '대한미디어'));
